  ...
  return (
    <main className="journalShell">
      ...
      <section className="panel chartPanel">
        <h3>Typical writing time by day of week</h3>
        ...
        <div className="hourHist">{hourly.map((h) => <div key={h.hour} className={`hourCol ${hourHover ? (hourHover.hour === h.hour ? "isHovered" : "isDimmed") : ""}`} onMouseEnter={(e) => setHourHover({ hour: h.hour, x: e.clientX, y: e.clientY })} onMouseMove={(e) => setHourHover({ hour: h.hour, x: e.clientX, y: e.clientY })} onMouseLeave={() => setHourHover(null)}><div className="hourBar" style={{ height: `${Math.max(8, (h.daysCount / maxHour) * 100)}%` }} /></div>)}</div>
